...
/** Builds one HTML entrypoint into its own outdir — shared by both the
 * income ledger's SPA and ส่งสลิป's SPA (Wave 2, docs/plan-audit-hub-
 * slips.md): two entirely separate origins/processes, but the same build
 * mechanics (Bun's HTML bundler + Tailwind v4 plugin), so this is
 * parameterized rather than duplicated. */
async function buildEntry(entrypoint: string, outdir: string): Promise<void> {
...
await buildEntry("src/client/index.html", "dist/client");
await buildEntry("src/slips/client/index.html", "dist/slips-client");
