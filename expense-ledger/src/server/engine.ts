...
/** Recovery for source sync's one full payment per AP row. Uses the durable
...
export async function findSourceApPayment(input: CreateApPaymentTransactionInput): Promise<string | null> {
  ...
  if (!res.success || !res.result) throw new Error('Cannot reconcile source payment');
  const matches = res.result.items.filter(r => extractTagIds(r).includes(String(tagId)));
  if (!matches.length) return null;
  if (matches.length !== 1) throw new Error('Multiple source payment transactions');
  const row = matches[0]!;
  if (row.sourceAmount !== input.amountSatang || deriveDateFromEngineTime(row.time) !== input.date
    ...
    throw new Error('Source payment transaction mismatch');
  }
  return String(row.id);
}

// Existing receipt integration keeps its public import name.
export const findReimbursementPayment = findSourceApPayment;
