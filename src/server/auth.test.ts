// B4 (Opus security review, 2026-08-03): an unset ACCESS_AUD in production
// must fail CLOSED (reject every request) rather than silently skipping the
// audience check. These tests prove the short-circuit fires BEFORE any
// network/JWKS work — proven by spying on global.fetch rather than needing
// a real signed JWT + live Cloudflare Access JWKS endpoint.

import { afterEach, beforeEach, describe, expect, test } from "bun:test";
import { _internal, verifyAccessJwt } from "./auth.ts";
...
describe("verifyAccessJwt — B4 fail-closed on unset ACCESS_AUD in production", () => {
  ...
  test("production + unset ACCESS_AUD: returns null WITHOUT ever fetching JWKS", async () => {
    ...
    expect(fetchCalls).toEqual([]); // the short-circuit fires before any network work at all
  });
  ...
  test("production + ACCESS_AUD set: proceeds past the fail-closed gate (attempts JWKS)", async () => {
    process.env.NODE_ENV = "production";
    ...
    // proving the fail-closed gate only fires for the unset-ACCESS_AUD case.
    expect(result).toBeNull();
